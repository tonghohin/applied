...
export function WeeklyActivityChart({ jobs }: { jobs: DashboardJob[] }) {
  ...
  const chartData = weekDays.map((day) => {
    ...
    const interviewing = jobs.filter(
      (job) => job.status === "interviewing" && isSameDay(job.updatedAt, day)
    ).length;
    ...
    return {
      ...
      failed,
      rejected,
      remaining: Math.max(0, found - applied - interviewing - failed - rejected),
    };
  });

  const totalApplied = chartData.reduce((acc, entry) => acc + entry.applied, 0);
  const totalInterviewing = chartData.reduce((acc, entry) => acc + entry.interviewing, 0);
      ...
      <CardHeader>
        ...
        <CardAction>
          <span className="font-mono text-muted-foreground text-xs">
            {totalApplied} applied · {totalInterviewing} interviewing · {totalFailed} failed ·{" "}
            {totalRejected} rejected · {totalFound} found
          </span>
        </CardAction>
      </CardHeader>
      <CardContent>
